...
# Index access
@test od4.order[1] == ("abcde", 1)
@test od4.order[120] == ("edcba", 120)

@test all([od.order[i] == (join(nthperm(split("abcde",""),i)),i) for i = 1:factorial(5)])
...
for i = factorial(5):-2:2
    delete!(od2, ks[i])         # delete by key
    delete!(od3.order, i)       # delete by numerical index
end
...
# first, last, reverse
@test first(od4) == od4.order[1] == ("abcde", 1)
@test last(od4) == od4.order[end] == ("edcba", 120)
@test reverse!(od4) == reverse(od5)
@test reverse!(od4) == od5

# push, pop, shift, unshift
@test push!(od4, ("hi", 0)) == ("hi", 0)
@test od4.order[length(od4)] == ("hi", 0)
@test unshift!(od4, ("bye", 100)) == ("bye", 100)
@test od4.order[1] == ("bye", 100)
